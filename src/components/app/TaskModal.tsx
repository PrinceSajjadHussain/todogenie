...
import type { Task, Subtask, Priority } from "@/types/todo";
...
interface Props {
  ...
  task?: Task | null; // Make task optional
...
export default function TaskModal({ open, onOpenChange, task, onClose }: Props) {
  ...
  const { updateTask, updateSubtask, addSubtask, translateTask, regenerateSubtasks, createTask } = useTasks();
  ...
  const handleCreateTask = (vals: { title: string; description?: string; due_date?: string | null; priority?: Priority | null; }) => {
    ...
    createTask.mutate({ title: vals.title, description: vals.description, due_date: vals.due_date || null, priority: vals.priority || null }, {
      onSuccess: () => {
        p.update({ id: p.id, title: "Task created — generating subtasks" });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[80vh] flex flex-col">
        ...
        {task ? (
          ...
          <TaskForm onSubmit={handleCreateTask} loading={createTask.isPending} />
        )}
      </DialogContent>
    </Dialog>
  );
